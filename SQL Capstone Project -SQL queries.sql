...
/*
Q9.Utilize SQL queries to segment customers based on demographics, account details, and transaction behaviours.
Ans1: please find below table with customers segments based on customers demographics.
GeographyLocation, GenderCategory, age_bracket, customers_count
France, Male, 31-50, 1861
France, Female, 31-50, 1517
Spain, Male, 31-50, 954
Germany, Male, 31-50, 897
Germany, Female, 31-50, 803
Spain, Female, 31-50, 739
France, Male, 18-30, 576
France, Female, 18-30, 462
France, Male, 50+, 316
France, Female, 50+, 282
Spain, Male, 18-30, 267
Germany, Male, 18-30, 245
Spain, Female, 18-30, 210
Germany, Female, 18-30, 208
Germany, Female, 50+, 182
Germany, Male, 50+, 174
Spain, Male, 50+, 167
Spain, Female, 50+, 140

Ans2: please find below table with customers segments based on customers account details.
joining_year, Tenure, Balance_range, customers_count
2019, 4, 75001-150000, 990
2019, 4, 0-75000, 791
2018, 5, 75001-150000, 759
2019, 3, 75001-150000, 696
2018, 5, 0-75000, 662
2017, 6, 75001-150000, 604
2016, 7, 75001-150000, 594
2019, 3, 0-75000, 522
2018, 4, 75001-150000, 501
2017, 6, 0-75000, 474
2017, 5, 75001-150000, 450
2016, 7, 0-75000, 447
2018, 4, 0-75000, 409
2016, 6, 75001-150000, 396
2017, 5, 0-75000, 390
2016, 6, 0-75000, 346
2019, 4, 150001-225000, 184
2018, 5, 150001-225000, 166
2017, 6, 150001-225000, 142
2019, 3, 150001-225000, 130
2016, 7, 150001-225000, 98
2018, 4, 150001-225000, 95
2017, 5, 150001-225000, 82
2016, 6, 150001-225000, 70
2017, 5, 225000+, 1
2018, 4, 225000+, 1

Ans3: please find below table with customers segments based on customers transaction behaviours.
NumOfProducts, customers_count
1, 5084
2, 4590
3, 266
4, 60
...
select g1.GeographyLocation,g.GenderCategory,case when c.age between '18' and '30' then '18-30' 
when c.age between '31' and '50' then '31-50' when c.age>50 then '50+' else 'NA' end age_bracket,
count(distinct c.CustomerId) customers_count from customerinfo c 
inner join bank_churn b on c.CustomerId=b.CustomerId
inner join  gender g on c.GenderID=g.GenderID
inner join  geography g1 on c.GeographyID=g1.GeographyID
...
select year(c.BankDOJ) joining_year,b.Tenure,case when b.Balance between '0' and '75000' then '0-75000'
when b.Balance between '75001' and '150000' then '75001-150000' when b.Balance between '150001' and '225000' then '150001-225000'
else '225000+' end Balance_range,count(distinct c.CustomerId) customers_count  from bank_churn b 
inner join customerinfo c on b.CustomerId=c.CustomerId
group by 1,2,3 order by 4 desc
;

select NumOfProducts,count(distinct CustomerId) customers_count from bank_churn
group by 1 order by 2 desc
;

/*
Q11.What is the current churn rate per year and overall as well in the bank. Can you suggest some insights to the bank about which kind of customers are more likely to churn and what are the different strategies that can be used to decrease the churn rate.
Ans: the current churn rate per year and overall as well in the bank were as following.
year, yearly_churn_rate, overall_churn_rate
2016, 19.27%, 20.37%
2017, 22.35%, 20.37%
2018, 20.21%, 20.37%
2019, 19.86%, 20.37%
...
with t1 as(select year(BankDOJ) year_,count(distinct CustomerId) total_customers_per_year from customerinfo
group by 1),t2 as (select year(c.BankDOJ) year_,count(distinct c.CustomerId) exited_customers from bank_churn b 
inner join customerinfo c on b.CustomerId=c.CustomerId
where Exited=1 group by 1),
t3 as (select t1.year_,t1.total_customers_per_year,sum(t1.total_customers_per_year) over() overall_customers,t2.exited_customers,
sum(t2.exited_customers) over() overall_exited_customers
from t1 inner join t2 on t1.year_=t2.year_)

select year_,round((exited_customers/total_customers_per_year)*100,2) yearly_churn_rate,
round((overall_exited_customers/overall_customers)*100,2) overall_churn_rate from t3
;
